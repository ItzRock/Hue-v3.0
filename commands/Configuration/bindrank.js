...
exports.run = async (client, message, args, level) => {
    ...
    if(message.settings.groupID.value == false) return message.channel.send(`${client.config.emojis.x} The config value: \`groupID\` needs to have a value.`)
    ...
    try {
        const rblxRoleInfo = await noblox.getRole(message.settings.groupID.value.toString(), parseInt(rank.rblx))
