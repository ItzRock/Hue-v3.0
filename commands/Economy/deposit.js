...
exports.run = async (client, message, args, level) => {
    ...
    if(info.BankCap === info.Bank) return message.channel.send(client.config.emojis.exclamation + " Your account is full! Please upgrade in order to deposit more.")
    ...
    if(info.Wealth - args[0] < 0) return message.channel.send(client.config.emojis.x + " Can't deposit more then you have!");
    const space = (info.BankCap - info.Bank)
    if(space - args[0] < 0) return message.channel.send(`${client.config.emojis.exclamation} There isnt enough space in your account! Space left: \`$${space}\``)
    if(args[0] <= 0) return message.channel.send(client.config.emojis.x +" Can't deposit less then zero.") 
    ...
    return message.channel.send(`${client.config.emojis.check} Deposited \`$${args[0]}\`, You now have \`$${(parseInt(info.Wealth) - parseInt(args[0]))}\` in your wallet, and \`$${newInfo.Bank}\` in your bank.`)
}
